...
async def outliers_index(
    ...
    # Datos según pestaña activa y ventana temporal
    league_outliers = []
    player_outliers = []
    
    if tab == "league":
        league_outliers = _get_league_outliers(db, season, limit=None, window=window)
    elif tab == "player":
        player_outliers = _get_player_outliers(db, season, limit=None, window=window)
...
async def api_league_outliers(
    ...
    outliers = _get_league_outliers(db, season, limit, window)
...
async def api_player_outliers(
    ...
    outliers = _get_player_outliers(db, season, limit, window, outlier_type)
    return JSONResponse(content={"data": outliers, "count": len(outliers)})
...
def _get_player_outliers(
    ...
    if effective_window == 'last_game':
        return _get_single_game_player_outliers(db, season, limit, effective_window, outlier_type)
    else:
        return _get_trend_player_outliers(db, season, limit, effective_window, outlier_type)


def _get_single_game_player_outliers(db, season, limit: Optional[int], window, outlier_type):
    query = (
    ...
    if outlier_type:
        ...
        query = query.order_by(PlayerOutlier.outlier_type.desc(), func.abs(PlayerOutlier.max_z_score).desc())
...
def _get_trend_player_outliers(db, season, limit: Optional[int], window, outlier_type):
    query = (
    ...
    if outlier_type:
        ...
        query = query.order_by(PlayerTrendOutlier.outlier_type.desc(), func.abs(PlayerTrendOutlier.max_z_score).desc())
    ...
    for trend, player in query.limit(limit).all():
        ...
        comp = trend.comparison_data.get(top_f, {}) if top_f else {}
        
        outlier_features = [{'feature': f, 'z_score': z} for f, z in sorted_feats[:3]]
        ...
        results.append({
            ...
            'primary_feat': top_f,
            'primary_val': comp.get('current_avg', 0),
            'primary_avg': comp.get('baseline_avg', 0),
            'window': window
        })
    return results
